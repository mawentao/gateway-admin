 ...
 * 用户权限表
 **/
class table_gwadmin_auth extends discuz_table
{
    public function __construct() {
		$this->_table = 'gwadmin_auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    public function set()
    {/*{{{*/
        $uid = gwadmin_validate::getNCParameter('uid','uid','integer');
        $auth = gwadmin_validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('gwadmin_auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	{/*{{{*/
        ...
        $auth  = gwadmin_validate::getNCParameter('auth','auth','integer');
        $key   = gwadmin_validate::getNCParameter('key','key','string',128);
        $sort  = gwadmin_validate::getOPParameter('sort','sort','string',1024,'uid');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#gwadmin#gwadmin_usergroup')->grouptitle($row['groupid']);
