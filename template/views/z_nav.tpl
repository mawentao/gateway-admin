...
<head>
  ...
  <script src="<%plugin_path%>/template/views/src/icon_dialog.js"></script>
  <%js_script%>
</head>
<body>
<form method="post" action="admin.php?action=plugins&operation=config&identifier=<%plugin_name%>&pmod=z_nav"
...
<script>
	...
	function addrow(domid,issub,item) {
        ...
        var displayorder = item.displayorder ? item.displayorder : menuidx;
		var code = '<tr id="menu-'+menuidx+'">'+
			  '<td>'+zd+
				'<input type="hidden" name="menuid[]" value="'+menuidx+'">'+
				'<input type="hidden" name="parentid[]" value="'+parentid+'">'+
			  '</td>'+
			  '<td class="td25"><input type="text" class="txt" name="displayorder[]" value="'+displayorder+'"></td>'+
